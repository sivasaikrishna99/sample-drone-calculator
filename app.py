...
if drone_type == "Agri Drone (Variable Payload)":
    ...
    water_kg_full = 0.0  # Surveillance drone has fixed payload

# --- Motor Thrust vs Current Table ---
...
# --- Helper function for phase Ah calculation ---
...
# --- Display results ---
st.header("Results")
st.metric("Takeoff Ah", f"{Ah_takeoff:.2f} Ah")
st.metric("Dispense Ah", f"{Ah_dispense:.2f} Ah")
st.metric("Landing Ah", f"{Ah_landing:.2f} Ah")
st.metric("Hovering time", f"{hovering_time:.2f} min")
st.metric("Max mission cycles", f"{max_cycles}")
st.metric("Total endurance", f"{total_endurance:.2f} min")
st.metric("Battery remaining", f"{battery_Ah - (Ah_per_cycle * max_cycles):.2f} Ah")
